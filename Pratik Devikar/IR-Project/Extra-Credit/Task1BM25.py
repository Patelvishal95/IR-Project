...
def write_scores_into_files(index, doc_score):
    f = open('D:\\IR-Project\\Pratik Devikar\\IR-Project\\Extra-Credit\\BM25_results_spelling_error\\BM25_scores_spelling_error_query_' + str(index) + '.txt', 'w')
    number_of_lines = min(100, len(doc_score))
    for i in range(number_of_lines):
        f.write(str(index) + " " + "Q0 " + doc_score[i][0][:-4] + " " + str(i + 1) + " " + str(
            doc_score[i][1]) + " " + "BM25_model_spelling_error_generator" + '\n')
    f.close()
