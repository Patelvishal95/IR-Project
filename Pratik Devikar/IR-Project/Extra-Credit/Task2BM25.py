...
def write_scores_into_files(index, doc_score):
    f = open('D:\\IR-Project\\Pratik Devikar\\IR-Project\\Extra-Credit\\BM25_results_spelling_corrected\\BM25_scores_spelling_corrected_query_' + str(index) + '.txt', 'w')
    number_of_lines = min(100, len(doc_score))
    for i in range(number_of_lines):
        f.write(str(index) + " " + "Q0 " + doc_score[i][0][:-4] + " " + str(i + 1) + " " + str(
            doc_score[i][1]) + " " + "BM25_model_spelling_corrected" + '\n')
    f.close()
...
def bm25_algorithm(unigram_inverted_dict, dl, avgdl, N, ni):
    query_index = 1
    f = open('Query_with_spelling_errors_corrected.txt', 'r')
    counter = 1
    for row in f:
