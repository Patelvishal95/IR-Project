...
def write_scores_into_files(index, doc_score):
    f = open('D:\\IR-Project\\Pratik Devikar\\IR-Project\\Task2\\BM25_Results_Query_Enrichment\\BM25_scores_query_enriched_query_' + str(index) + '.txt', 'w')
    number_of_lines = min(100, len(doc_score))
    for i in range(number_of_lines):
        f.write(str(index) + " " + "Q0 " + doc_score[i][0][:-4] + " " + str(i + 1) + " " + str(
            doc_score[i][1]) + " " + "BM25_model_query_enriched" + '\n')
    f.close()
...
def bm25_algorithm(unigram_inverted_dict, dl, avgdl, N, ni):
    query_index = 1
    f = open('Refined_Query_Enriched.txt', 'r')
    counter = 1
    for row in f:
